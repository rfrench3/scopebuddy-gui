...
class MainWindow(QMainWindow): 
    ...
    def get_gamescope_args(self) -> str: #search for the config file, return args if it exists
        # Check if the config file exists, make it if not
        if not os.path.exists(os.path.expanduser('~/.config/scopebuddy/scb.conf')):
            ...
            if verify_dependencies_present(['gamescope', 'scopebuddy']):
                print("dependencies present, generating basic file...")
                #generates new config file with default values
                os.makedirs(os.path.dirname(os.path.expanduser('~/.config/scopebuddy/scb.conf')), exist_ok=True)
                ...
                return None # file won't do anything so refuse to create it
